...
async function run() {
    ...
    // Find all blocks
    const bikes = [];
    const blockRegex = /id:\s*"([^"]+)",[\s\S]*?slug:\s*"([^"]+)"/g;
    let bMatch;
    while ((bMatch = blockRegex.exec(content)) !== null) {
        bikes.push({ id: bMatch[1], slug: bMatch[2] });
    }
    
    for (const bike of bikes) {
        console.log(`Processing ${bike.id} (${bike.slug})...`);
        const html = await fetchHtml(`https://honsupermotos.com/${bike.slug}`);
        ...
        const slugIdx = content.indexOf(`id: "${bike.id}"`);
        ...
        console.log(`  -> Updated colors for ${bike.id}`);
    }
